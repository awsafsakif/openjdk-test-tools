...
module.exports = async ( req, res ) => {
    ...
    for (benchmarkIndex in benchmarks) {
        ...
        for (item in platforms) {
            ...
            const latestDate = Math.max.apply(Math, exceedFilter.map(function(o) { return parseInt(o.jdkDate.replace(/[\s()-]+/gi, '')); }));
            // Remove all runs that are not the latest
            const dateFilter = exceedFilter.filter(entry => parseInt(entry.jdkDate.replace(/[\s()-]+/gi, '')) === latestDate);
